...
void readinp(struct set_host *seth){
    ...
    while (item) {
        ...
        if (NULL != cJSON_GetObjectItem(item, "type_seed")) {
            list = cJSON_GetObjectItem(item, "type_seed");
            if (list->type == cJSON_Number) {
                seth->type_seed = list->valueint;
            }
        }


        if (NULL != cJSON_GetObjectItem(item, "if_printtraj")) {
    ...
    // printf("Natom_mole: %d\n", seth->Natom_mole);
    // printf("Nstate_mole: %d\n", seth->Nstate_mole);
    // printf("path_qmkeyword_mole: %s\n", seth->path_qmkeyword_mole);
