...
void V_SBM(double *R, double H[2][2], int forcetype) {
    switch (forcetype) {
        case 0:
            H[0][0] = eps_SBM + dot_product(c_SBM, R) + 0.5 * dot_product(pow(omega_SBM, 2), pow(R, 2));
            H[0][1] = delta_SBM;
            H[1][1] = -eps_SBM - dot_product(c_SBM, R) + 0.5 * dot_product(pow(omega_SBM, 2), pow(R, 2));
            H[1][0] = delta_SBM;
            break;
        case 1:
            H[0][0] = eps_SBM + dot_product(c_SBM, R);
            H[0][1] = delta_SBM;
            H[1][1] = -eps_SBM - dot_product(c_SBM, R);
            H[1][0] = delta_SBM;
            break;
        default:
...
void dV_SBM(double *R, double dH[2][2][N_bath_SBM], int forcetype) {
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
    ...
    switch (forcetype) {
        case 0:
            for (int j = 0; j < N_bath_SBM; j++) {
                dH[0][0][j] += c_SBM[j] + pow(omega_SBM[j], 2) * R[j];
                dH[1][1][j] -= c_SBM[j] + pow(omega_SBM[j], 2) * R[j];
            }
            break;
        case 1:
            for (int j = 0; j < N_bath_SBM; j++) {
