...
void parameter_mole(double *mass, struct set_host *setm) {
    ...
    setm->atomname_mole = {"Bq","H ","He",  // Bq, 1-~2  
    ...
    setm->atommass_mole = { 0.0,	1.00794,	4.002602, // & !Bq; 1-2
...
void dV_mole(double *R, double *dH, int forcetype, struct set_host *setm) {
    // for (int i = 0; i < 2; i++) {
    //     for (int j = 0; j < 2; j++) {
    //         for (int k = 0; k < N_bath_mole; k++) {
    //             dH[i*2*N_bath_mole+j*N_bath_mole+k] = 0.0;
    //         }
    //     }
    // }
    // memset(dH,0,4*setm->N_bath_mole*sizeof(double));

    // switch (forcetype) {
    //     case 0:
    //         for (int j = 0; j < setm->N_bath_mole; j++) {
    //             dH[0*2*setm->N_bath_mole+0*setm->N_bath_mole+j] = setm->c_mole[j]+setm->omega_mole[j]* setm->omega_mole[j] * R[j];
    //             dH[1*2*setm->N_bath_mole+1*setm->N_bath_mole+j] = -setm->c_mole[j]+setm->omega_mole[j]* setm->omega_mole[j] * R[j];
              
    //         }
    //         break;
    //     case 1:
    //         for (int j = 0; j < setm->N_bath_mole; j++) {
    //             dH[0*2*setm->N_bath_mole+0*setm->N_bath_mole+j] = setm->c_mole[j];
    //             dH[1*2*setm->N_bath_mole+1*setm->N_bath_mole+j] = -setm->c_mole[j];
    //         }
    //         break;
    //     default:
    //         fprintf(stderr, "ERROR: unknown forcetype\n");
